...
[{
    ...
    "URL": ["https://leg.colorado.gov/sites/default/files/images/olls/crs{year}-title-{lpad}{title}.pdf#search={lpad}{title}-{article}-{section}."],
    "defaults": {
        "year": "2020"
    },
    "substitutions": [{
    ...
    "name": "Delaware Code",
    "regex": "([Tt]it(le|\\.) )?(?<title>\\d{1,2}),? (Del(aware|\\.)|DE) ?C(ode|\\.) ((&sect;|&#167|§){1,2}|[Ss]ect?(ions?|s?\\.))? ?(?<section>(\\d[\\w.]*\\w|\\d))(?<subsection>(\\(\\w+\\))+)?",
    ...
    "name": "CourtListener",
    "regex": "(?<=\\b)(?<volume>\\d+) (?<reporter>S\\. ?Ct\\.|L\\. ?Ed\\.( ?2d)) (?<page>\\d+)\\b(,?( at)? (?<pincite>\\d+)(?! \\w)(([-‑–]| to | through )(?<pincite_end>\\d+))?\\b)?",
    "URL": ["https://www.courtlistener.com/c/{reporter}/{volume}/{page}"],
    "mutations": [{
        "token": "reporter",
        "omit": " "
    }],
...
function getMatch(query) {
  for (var i = 0; i < schemas.length; i++) {
    ...
    if (match) {
      console.log('"' + query + '" matched regex for ' + schema['name'] + ', detecting these tokens:'); 
...
function handleDefaults(match) {
  ...
  for (d in schema.defaults) {
    if (!keys[d]) {
      console.log(d + ' was not specified, so it will be set to "' + schema.defaults[d] + '" by default');
...
function handleMutations(match) {
  ...
  for (m in schema.mutations) {
    ...
    console.log('performed string mutation to turn ' + mutation['token'] + ' "' + keys[mutation['token']] + '" into "' + key + '"');
...
function handleSubstitutions(match) {
  ...
  for (var s in schema.substitutions) {
    ...
    if (newKey === undefined) {
      ...
      console.log('tried looking up ' + '"' + keys[sub['token']] +'" in a table, but could not find a corresponding value');
      if (('allowUnmatched' in sub) & sub['allowUnmatched']) {
        ...
        console.log('looked up ' + sub['token'] + ' "' + keys[sub['token']] + '" in a table to set ' + outputToken + ' to "' + newKey + '"');
...
function updateUrlParts(match) {
  let {schema, keys} = match;
  console.log("filling in placeholders in each part of the URL template...");
...
function buildUrl(match) {
  ...
  for (p in schema.URL) {
    ...
    if (!part.match(missingPlaceholder)) {
      ...
      console.log('did not add "' + part + '" because it contains a placeholder that was not filled in');
    }
  }
...
United States Constitution | [constitution.congress.gov](https://constitution.congress.gov) | [view regex](https://regexper.com#%28U%5C.%3F%20%3FS%5C.%3F%7CUnited%20States%29%20%3FConst%28itution%7C%5C.%29%2C%3F%20%28%5BAa%5Drt%28icle%7C%5C.%29%20%28%5B%5CdIViv%5D%7B1%2C3%7D%29%7C%5BAa%5Dm%28endment%7C%28end%7Cdt%3F%29%3F%5C.%29%20%28%5B%5CdXIVxiv%5D%7B1%2C3%7D%29%29%28%28%2C%3F%20%28%28%26sect%3B%7C%26%23167%7C%C2%A7%29%7B1%2C2%7D%7C%5BSs%5Dect%3F%28ions%3F%7Cs%3F%5C.%29%29%20%3F%28%5Cd%2B%7C%5BIVXivx%5D%7B1%2C7%7D%29%29%28%28%2C%3F%20%28%5BCc%5Dl%28ause%7C%5C.%29%20%3F%28%5Cd%2B%29%29%29%29%3F%29%3F)
...
CourtListener | [www.courtlistener.com](https://www.courtlistener.com) | [view regex](https://regexper.com#%28%5Cd%2B%29%20%28S%5C.%20%3FCt%5C.%7CL%5C.%20%3FEd%5C.%28%20%3F2d%29%29%20%28%5Cd%2B%29%5Cb%28%2C%3F%28%20at%29%3F%20%28%5Cd%2B%29%28%28%5B-%E2%80%91%E2%80%93%5D%7C%20to%20%7C%20through%20%29%28%5Cd%2B%29%29%3F%5Cb%29%3F)
